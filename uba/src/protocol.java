    ...
    public void processing(byte[] jcmResponse){
        ...
        switch(jcmResponse[2]){
            ...
            case SR_IDLING: //0x11 IDLING
            	//System.out.println(baitsToString("protocol processing IDLING", jcmResponse));            	
            	//id003_format((byte)5, (byte) STATUS_REQUEST, jcmMessage); //STATUS_REQUEST
            	break;
            case SR_ACCEPTING: //0x12 ACCEPTING
            	if(mostrar) System.out.println(baitsToString("protocol processing ACCEPTING", jcmResponse));
            	id003_format((byte)5, (byte) 0x11, jcmMessage,true); //STATUS_REQUEST
            	break;
            case SR_ESCROW: // 0x13 ESCROW
            	if(mostrar) System.out.println(baitsToString("protocol processing ESCROW", jcmResponse));
            ...
            case SR_STACKING: //0x14 STACKING +DATA
            	if(mostrar) System.out.println(baitsToString("protocol processing STACKING", jcmResponse));
            ...
            case SR_VEND_VALID: //0x15 VEND_VALID
            	if(mostrar) System.out.println(baitsToString("protocol processing VEND_VALID", jcmResponse));
            	id003_format((byte)5, ACK, jcmMessage,true); //ACK
            	break;
            case SR_STACKED: // 0x16 STACKED  +DATA
            	if(mostrar) System.out.println(baitsToString("protocol processing STACKED", jcmResponse));
            	id003_format((byte)5, (byte) 0x11, jcmMessage,true); //STATUS_REQUEST
            	MyClass.fireMyEvent(new MyEvent("clearbill"));
            	break;
            case SR_REJECTING: // 0x17 REJECTING
            	if(mostrar) System.out.println(baitsToString("protocol processing REJECTING", jcmResponse));
            ...
            case SR_RETURNING: //0x18 RETURNING
            	if(mostrar) System.out.println(baitsToString("protocol processing RETURNING", jcmResponse));
            	id003_format((byte)5, (byte) 0x11, jcmMessage,true); //STATUS_REQUEST
            	MyClass.fireMyEvent(new MyEvent("clearbill"));
            	break;
            	
            case SR_HOLDING: // 0x19 HOLDING            	
            	if(mostrar) System.out.println(baitsToString("protocol processing HOLDING", jcmResponse));
            ...
            case SR_INITIALIZE: // 0x1B INITIALIZE
            	if(mostrar) System.out.println(baitsToString("protocol processing INITIALIZE", jcmResponse));
            	id003_format((byte)5, (byte) 0x88, jcmMessage,true); //SSR_VERSION_REQUEST
            	break;
            	
            case SR_PAYING: // 0x20 PAYING
            	if(mostrar) System.out.println(baitsToString("protocol processing PAYING", jcmResponse));            	
            	break;
            	
            case SR_COLLECTING: // 0x21 COLLECTING
            	if(mostrar) System.out.println(baitsToString("protocol processing COLLETING", jcmResponse));                        	
            	break;
            	
            case SR_COLLECTED: // 0x22 COLLECTED + DATA
            	if(mostrar) System.out.println(baitsToString("protocol processing COLLECTED", jcmResponse));
            ...
            case SR_PAY_VALID: //0x23 PAY VALID
            	if(mostrar) System.out.println(baitsToString("protocol processing PAY VALID", jcmResponse)); 
            	id003_format((byte)5, ACK, jcmMessage,true); // ACK
            	break;
            ...
            case SR_RETURN_TO_BOX: //0x25 RETURN TO BOX
            	if(mostrar) System.out.println(baitsToString("protocol processing RETURN TO BOX", jcmResponse)); 
            	id003_format((byte)5, ACK, jcmMessage,true); // ACK
            	break;
            	
            case SR_RETURN_PAY_OUT_NOTE: //0x26 RETURN PAY OUT NOTE +DATA
             	if(mostrar) System.out.println(baitsToString("protocol processing RETURN PAY OUT NOTE", jcmResponse)); 
            	id003_format((byte)5, ACK, jcmMessage,true); // ACK
            	break;
            ...
            case SR_POWER_UP_WITH_BILL_IN_STACKER: //0x42 POWER_UP_WITH_BILL_IN_STACKER / MOTOR_ERROR            	
            	if(mostrar) System.out.println(baitsToString("protocol processing SR_POWER_UP_WITH_BILL_IN_STACKER", jcmResponse));
            ...
            case SR_E_UNCONNECTED: //0x00  UNCONNECTED
            	if(mostrar) System.out.println(baitsToString("protocol processing SR_E_UNCONNECTED", jcmResponse));            	
            	break;	
            	
            case SR_E_NORMAL: //0x10  SR_E_NORMAL + DATA
            	if(mostrar) System.out.println(baitsToString("protocol processing SR_E_NORMAL", jcmResponse));            	
            	break;	
            /*  TODO: VALIDAR SI ES EXTENSION	
            case SR_E_EMPTY: //0x11  SR_E_EMPTY
            	if(mostrar) System.out.println(baitsToString("protocol processing SR_E_EMPTY", jcmResponse));            	
            	break;	
            */	
            case SR_ERR_STACKER_FULL: //0x43 STACKER_FULL
            	if(mostrar) System.out.println(baitsToString("protocol processing STACKER FULL", jcmResponse));            	
            	break;
            case SR_ERR_STACKER_OPEN: //0x44 SR_ERR_STACKER_OPEN
            	if(mostrar) System.out.println(baitsToString("protocol processing SR_ERR_STACKER_OPEN", jcmResponse));            	
            	break;
            case SR_ERR_JAM_IN_ACCEPTOR: //0x45 JAM_IN_ACCEPTOR
            	if(mostrar) System.out.println(baitsToString("protocol processing SR_ERR_JAM_IN_ACCEPTOR", jcmResponse));            	
            	break;
            case SR_ERR_JAM_IN_STACKER: //0x46 SR_ERR_JAM_IN_STACKER
            	if(mostrar) System.out.println(baitsToString("protocol processing SR_ERR_JAM_IN_STACKER", jcmResponse));            	
            	break;
            	
            case SR_ERR_PAUSE: //0x47 SR_ERR_PAUSE
            	if(mostrar) System.out.println(baitsToString("protocol processing SR_ERR_PAUSE", jcmResponse));            	
            	break;
            	
            case SR_ERR_CHEATED: //0x48 SR_ERR_CHEATED
            	if(mostrar) System.out.println(baitsToString("protocol processing SR_ERR_CHEATED", jcmResponse));            	
            	break;	
            	
            case SR_ERR_FAILURE: //0x49 SR_ERR_FAILURE + DATA
            	if(mostrar) System.out.println(baitsToString("protocol processing SR_ERR_FAILURE", jcmResponse));            	
            	//TODO: Checar el DATA
            	break;	
            	
            case SR_ERR_COMMUNICATION_ERROR: //0x4A SR_ERR_COMMUNICATION_ERROR
            	if(mostrar) System.out.println(baitsToString("protocol processing SR_ERR_COMMUNICATION_ERROR", jcmResponse));            	
            	//TODO: Checar el DATA
            	break;	           	
            	
            case SCR_ENABLE_DENOM: // 0xC0 SCR_ENABLE_DENOM
            	if(mostrar) System.out.println(baitsToString("protocol processing SCR_ENABLE_DENOM", jcmResponse));
            	//TODO: Checar el DATA            		
            	id003_format((byte)7, (byte) 0xC1, jcmMessage,true); // CMD_SECURITY_DENOMI
            	break;
            case SCR_SECURITY_DENOM: //0xC1 SCR_SECURITY_DENOM
            	if(mostrar) System.out.println(baitsToString("protocol processing SCR_SECURITY_DENOM", jcmResponse));
            	//TODO: Checar el DATA            		
            	id003_format((byte)6, (byte) 0xC4, jcmMessage,true); // CMD_DIRECTION
            	break;
            case SCR_COMMUNICATION_MODE: //0xC2 CMD_INHIBIT_ACCEPTOR
            	if(mostrar) System.out.println(baitsToString("protocol processing CMD INHIBIT ACCEPTOR", jcmResponse));
            	//TODO: Checar el DATA            		
            	id003_format((byte)5, (byte) 0x83, jcmMessage,true); // SSR_INHIBIT_ACCEPTOR
            	break;
            case SCR_INHIBIT: //0xC3 SCR_INHIBIT
            	if(mostrar) System.out.println(baitsToString("protocol processing CMD SCR_INHIBIT ACCEPTOR", jcmResponse));
            	//TODO: Checar el DATA            		
            	id003_format((byte)5, (byte) 0x83, jcmMessage,true); // SSR_INHIBIT_ACCEPTOR
            	break;
            ...
            case SSRR_VERSION_INFORMATION: //0x88 SSRR_VERSION_INFORMATION  (FIRMWARE)
            	if(mostrar) System.out.println(baitsToString("protocol processing FIRMWARE", jcmResponse));
            	id003_format((byte)5, (byte) 0x89, jcmMessage,true); // SSR_BOOT_VERSION_REQUEST
            	System.arraycopy(jcmResponse, 3, protocol.version, 0,jcmResponse[1]-5);
